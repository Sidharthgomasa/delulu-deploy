...
if uploaded:
    ...
    if df.empty:
        st.error("Bro this file format is not supported 😭")
        st.stop()
    ...
    total_time = (df["Datetime"].max() - df["Datetime"].min()).days
    ...
    with col4: st.markdown(f'<div class="card">⏳<div class="metric">{total_time} days</div>Chat Time</div>', unsafe_allow_html=True)
